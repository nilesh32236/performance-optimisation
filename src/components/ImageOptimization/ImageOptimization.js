...
const ImageOptimization = ({
	...
	const availablePostTypesOptions =
		uiData.availablePostTypes?.map(pt => ({
	...
	return (
		<div className="wppo-settings-form wppo-image-optimization-settings">
			...
			<p className="wppo-section-description">
				{translations.imageOptimizationDesc ||
					"Configure lazy loading, image conversion, and preloading strategies for your site's images."}
			</p>
			...
			<div className="wppo-form-section">
				...
				<div className="wppo-field-group wppo-checkbox-option">
					...
					/>
					<label htmlFor="lazyLoadImages">
						{translations.lazyLoadImages || 'Lazy Load Images'}
					</label>
				</div>
				{imgOptSettings.lazyLoadImages && (
					<div className="wppo-sub-fields">
						<label htmlFor="excludeImages" className="wppo-label">
							{translations.excludeImages ||
								'Exclude Images/Keywords from Lazy Loading:'}
						</label>
						<textarea
							id="excludeImages"
							...
							onChange={e =>
								handleChange('excludeImages', e.target.value, 'textarea')
							}
							rows="3"
							placeholder={
								translations.excludeImagesHelpText ||
								'e.g., /logo.png, critical-image-class, /path/to/specific-slider/'
							}
						/>
						<p className="wppo-option-description">
							{translations.lazyLoadImagesDesc ||
								'Prevents specific images from being lazy-loaded. Useful for above-the-fold content.'}
						</p>

						<label
							htmlFor="excludeFistImages"
						...
						/>
						<p className="wppo-option-description">
							{translations.excludeFistImagesDesc ||
								'Number of images from the top of the page to exclude from lazy loading.'}
						</p>

						<div
							className="wppo-field-group wppo-checkbox-option"
							style={{ marginTop: '15px' }}
						>
							<input
								type="checkbox"
								...
								onChange={e =>
									handleChange('replacePlaceholderWithSVG', e.target.checked)
								}
							/>
							<label htmlFor="replacePlaceholderWithSVG">
								{translations.replaceImgToSVG ||
									'Use SVG Placeholders for Lazy Loaded Images'}
							</label>
						</div>
						<p className="wppo-option-description" style={{ marginTop: '-10px' }}>
							{translations.replaceImgToSVGDesc ||
								'Replaces the low-quality image placeholder with a lightweight SVG, matching image dimensions.'}
						</p>
					</div>
				)}
				...
				>
					...
					/>
					<label htmlFor="lazyLoadVideos">
						{translations.lazyLoadVideos || 'Lazy Load Videos (iframes & video tags)'}
					</label>
				</div>
				{imgOptSettings.lazyLoadVideos && (
					<div className="wppo-sub-fields">
						<label htmlFor="excludeVideos" className="wppo-label">
							{translations.excludeVideos ||
								'Exclude Videos/Keywords from Lazy Loading:'}
						</label>
						<textarea
							id="excludeVideos"
							className="wppo-text-area-field"
							...
							onChange={e =>
								handleChange('excludeVideos', e.target.value, 'textarea')
							}
							rows="3"
							placeholder={
								translations.excludeVideosHelpText ||
								'e.g., youtube.com/embed/specific-id, vimeo-player-class'
							}
						/>
						<p className="wppo-option-description">
							{translations.excludeVideosDesc ||
								'Prevents specific videos (iframes or video tags with matching src/class) from being lazy-loaded.'}
						</p>
					</div>
				)}
			</div>
			...
			<div className="wppo-form-section">
				...
				<div className="wppo-field-group wppo-checkbox-option">
					...
					/>
					<label htmlFor="convertImg">
						{translations.convertImg || 'Enable Next-Gen Image Conversion (WebP/AVIF)'}
					</label>
				</div>
				{imgOptSettings.convertImg && (
					<div className="wppo-sub-fields">
						<label htmlFor="conversionFormat" className="wppo-label">
							{translations.conversionFormat || 'Preferred Conversion Format:'}
						</label>
						<select
							id="conversionFormat"
							...
							onChange={e =>
								handleChange('conversionFormat', e.target.value, 'select')
							}
						>
							<option value="webp">{translations.webpOnly || 'WebP Only'}</option>
							<option value="avif">
								{translations.avifOnly ||
									'AVIF Only (fallback to WebP/Original if not supported)'}
							</option>
							<option value="both">
								{translations.bothFormats ||
									'Both (Serve AVIF if supported, else WebP, else Original)'}
							</option>
						</select>
						<p className="wppo-option-description">
							{translations.conversionFormatDesc ||
								'AVIF generally offers better compression but has less browser support than WebP.'}
						</p>
						...
						<label
							...
							style={{ marginTop: '15px' }}
						>
							{translations.excludeConvertImages ||
								'Exclude Images/Keywords from Conversion:'}
						</label>
						<textarea
							...
							onChange={e =>
								handleChange('excludeConvertImages', e.target.value, 'textarea')
							}
							rows="3"
							placeholder={
								translations.excludeConvertImagesHelpText ||
								'e.g., /gifs/, specific-logo.png'
							}
						/>
						<p className="wppo-option-description">
							{translations.excludeConvertImagesDesc ||
								'Images matching these keywords or paths will not be converted.'}
						</p>

						<label
							htmlFor="imgBatchSize"
							className="wppo-label"
							style={{ marginTop: '15px' }}
						>
							{translations.imgBatchSize || 'Image Conversion Batch Size:'}
						</label>
						<input
							type="number"
							id="imgBatchSize"
							...
							onChange={e =>
								handleChange('batch', parseInt(e.target.value, 10) || 50, 'number')
							}
							min="1"
						...
						/>
						<p className="wppo-option-description">
							{translations.imgBatchSizeDesc ||
								'Number of images processed per cron run. Higher values process faster but use more resources.'}
						</p>
						<p className="wppo-option-description wppo-warning-text">
							<FontAwesomeIcon icon={faExclamationTriangle} />{' '}
							{translations.ensureGdImagick ||
								'Ensure your server has GD library with WebP/AVIF support, or Imagick extension, for conversion to work.'}
						</p>
					</div>
				)}
			</div>
			...
			<div className="wppo-form-section">
				...
				<div className="wppo-field-group wppo-checkbox-option">
					...
					/>
					<label htmlFor="preloadFrontPageImages">
						{translations.preloadFrontPageImg ||
							'Preload Critical Images on Front Page'}
					</label>
				</div>
				{imgOptSettings.preloadFrontPageImages && (
					<div className="wppo-sub-fields">
						<label htmlFor="preloadFrontPageImagesUrls" className="wppo-label">
							{translations.preloadFrontPageImgUrl ||
								'Front Page Image URLs to Preload (one per line):'}
						</label>
						<textarea
							...
							rows="3"
							placeholder={
								translations.preloadFrontPageImgUrlHelpText ||
								'e.g., /wp-content/uploads/hero.jpg\nmobile:/uploads/hero-sm.jpg'
							}
						/>
						<p className="wppo-option-description">
							{translations.preloadFrontPageImgDesc ||
								'Specify images critical for the first view of your front page. Use mobile:/desktop: prefixes for device-specific preloads.'}
						</p>
					</div>
				)}
				...
				>
					...
					/>
					<label htmlFor="preloadPostTypeImage">
						{translations.preloadPostTypeImg ||
							'Preload Featured Images for Specific Post Types'}
					</label>
				</div>
				{imgOptSettings.preloadPostTypeImage && (
					<div className="wppo-sub-fields">
						<label htmlFor="selectedPostType" className="wppo-label">
							{translations.selectPostTypes || 'Select Post Types:'}
						</label>
						<Select
							id="selectedPostType"
							...
							onChange={selected =>
								handleMultiSelectChange('selectedPostType', selected)
							}
						...
						<p className="wppo-option-description">
							{translations.preloadPostTypeImgDesc ||
								'Featured images for selected post types will be preloaded on their single views.'}
						</p>
						...
						>
							{translations.excludePostTypeImgUrl ||
								'Exclude Featured Images from Preloading (Keywords/Paths):'}
						</label>
						<textarea
							...
							onChange={e =>
								handleChange('excludePostTypeImgUrl', e.target.value, 'textarea')
							}
							rows="2"
							placeholder={
								translations.excludePostTypeImgUrlHelpText ||
								'e.g., small-thumbnail.jpg'
							}
						/>
						<p className="wppo-option-description">
							{translations.excludePostTypeImgUrlDesc ||
								'Featured images matching these will not be preloaded.'}
						</p>
						...
						>
							{translations.maxWidthImgSize ||
								'Max Width for Preloaded Srcset Images (px, 0 for no limit):'}
						</label>
						<input
							type="number"
							id="maxWidthImgSize"
						...
						/>
						<p className="wppo-option-description">
							{translations.maxWidthImgSizeDesc ||
								'Limits preloading of srcset images to those at or below this width.'}
						</p>
						...
						>
							{translations.excludeSize ||
								'Exclude Specific Srcset Image Widths from Preloading (px, one per line):'}
						</label>
						<textarea
							id="excludeSize"
							className="wppo-text-area-field"
						...
						/>
						<p className="wppo-option-description">
							{translations.excludeSizeDesc ||
								'Images with these exact widths in srcset will not be preloaded.'}
						</p>
					</div>
				)}
			</div>
