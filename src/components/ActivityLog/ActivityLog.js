...
const ActivityLog = ({
	...
	const fetchActivities = useCallback(
		async (pageToFetch = 1, loadMore = false) => {
			setIsFetching(true);
			setError('');
			try {
				...
				if (result.success && result.data) {
					setActivities(prevActivities =>
						loadMore
							? [...prevActivities, ...result.data.activities]
					...
					throw new Error(
						result.data?.message ||
							result.message ||
							translations.failedFetchActivities ||
		...
		[apiUrl, nonce, perPage, translations]
	); // translations added if error messages use it
	...
	return (
		<div className="wppo-settings-form wppo-recent-activities-container">
			{' '}
			{/* Changed class for better distinction */}
			...
			<p className="wppo-section-description">
				{translations.activityLogDesc ||
					'View recent actions and events related to the Performance Optimisation plugin.'}
			</p>
			{error && (
					...
					<ul>
						{activities.map((activity, index) => (
							...
							<li key={activity.id || index}>
								<span
									className="wppo-activity-timestamp"
									style={{
										marginRight: '10px',
										color: '#777',
										fontSize: '0.9em',
								...
								<span
									className="wppo-activity-description"
			...
			{!isFetching && currentPage < totalPages && (
				<div style={{ textAlign: 'center', marginTop: '20px' }}>
					<button className="wppo-button" onClick={handleLoadMore} disabled={isFetching}>
