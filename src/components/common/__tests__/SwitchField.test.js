...
describe('SwitchField', () => {
	...
	it('renders label and toggle correctly', () => {
		...
		expect(screen.getAllByText('Toggle Feature').length).toBeGreaterThan(0);
	...
	it('renders description if provided', () => {
		render(<SwitchField {...defaultProps} description="Feature description here." />);
	...
	it('does not render label in wrapper if showLabel is false and no description is present', () => {
		const { container } = render(<SwitchField {...defaultProps} showLabel={false} />);
