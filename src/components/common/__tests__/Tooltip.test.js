...
describe('Tooltip', () => {
	it('renders children without tooltip if content is empty', () => {
		...
		expect(screen.queryByText('Hover Me')?.closest('.wppo-tooltip-container')).not.toBeInTheDocument();
	});
	...
	it('renders tooltip content and children', () => {
		...
		expect(screen.getByText('Tooltip Content')).toHaveClass('wppo-tooltip-content');
	});
