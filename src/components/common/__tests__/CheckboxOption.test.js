...
describe('CheckboxOption', () => {
	...
	it('renders label and checkbox correctly', () => {
		...
		const checkbox = screen.getByRole('checkbox', { name: /Enable Feature/i });
	...
	it('renders description if provided and links via aria-describedby', () => {
		render(<CheckboxOption {...defaultProps} description="This is a feature description." />);
		const descriptionEl = screen.getByText('This is a feature description.');
	...
	it('renders nested content and textarea when checked', () => {
		...
		const textarea = screen.getByRole('textbox', { name: /Enter details/i });
