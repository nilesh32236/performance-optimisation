 ...
 * @param {import('react').Node} [props.status]      Optional status badge / indicator.
 * @param {import('react').Node} [props.actions]     Buttons rendered on the right.
 * @param {import('react').Node} [props.children]    Extra content below the header row.
 */
	...
	<div className="wppo-feature-header">
		<div className="wppo-feature-header__main">
			<div className="wppo-feature-header__title">
				...
				{ status && (
					<div className="wppo-feature-header__status">{ status }</div>
				) }
			</div>
			{ actions && (
