	...
	return (
		<div className="wppo-feature-toggle" role="group" aria-labelledby={`feature-title-${id}`}>
			<div className="wppo-feature-content">
				...
				<p id={`feature-desc-${id}`} className="wppo-feature-description">
					{description}
				</p>

				{longDescription && (
					<p id={`feature-long-desc-${id}`} className="wppo-feature-long-description">
						{longDescription}
					</p>
				)}
			</div>
			...
			<div className="wppo-feature-control">
				<label className="wppo-toggle-switch" htmlFor={`toggle-${id}`}>
					<input
						...
						aria-describedby={
							longDescription
								? `feature-desc-${id} feature-long-desc-${id}`
								: `feature-desc-${id}`
						}
