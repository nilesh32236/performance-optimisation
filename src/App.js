...
function App({ adminData }) {
	...
	const saveSettingsForTab = useCallback(
		async tabKey => {
			if (!settings[tabKey]) {
	...
	const menuItems = [
		...
		{
			id: 'fileOptimization',
			label: translations.fileOptimization || 'File Optimization',
			icon: faFileCode,
			component: FileOptimization,
			tabKey: 'file_optimisation',
			hasSaveButton: true,
		},
		{
			id: 'imageOptimization',
			label: translations.imageOptimization || 'Image Optimization',
			icon: faImages,
			component: ImageOptimization,
			tabKey: 'image_optimisation',
			hasSaveButton: true,
		},
		{
			id: 'preloadSettings',
			label: translations.preloadSettings || 'Preload & Preconnect',
			icon: faRocket,
			component: PreloadSettings,
			tabKey: 'preload_settings',
			hasSaveButton: true,
		},
		{
			id: 'tools',
			label: translations.tools || 'Tools',
			icon: faTools,
			component: Tools,
			hasSaveButton: false,
		},
		{
			id: 'activityLog',
			label: translations.activityLog || 'Activity Log',
			icon: faHistory,
	...
	if (!adminData) {
		// Or a more sophisticated loading state check
		return (
			<div className="wppo-container">
				...
				>
					<p>
						{translations.loadingSettings ||
							'Loading Performance Optimisation Settings...'}
					</p>
	...
	return (
		<div className="wppo-admin-app-wrapper">
			...
			<div className="wppo-container">
				<button
					...
					aria-label={
						sidebarOpen
							...
							: translations.openMenu || 'Open menu'
					}
				...
				<aside
					className={`wppo-sidebar ${sidebarOpen ? 'wppo-sidebar--open' : 'wppo-sidebar--hide'}`}
				>
