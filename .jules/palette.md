...
## 2026-06-20 - Use Native Theme Variables for Backgrounds
**Learning:** Hardcoded white backgrounds (`#fff`) in SCSS components (like dialogs, forms, and audit cards) can clash with customized WordPress admin themes and make dark mode or alternative light themes look inconsistent. Additionally, `#ffffff` or `#fff` literals prevent deeper dynamic adaptations within the plugin.
**Action:** Replace hardcoded `#fff` and `#ffffff` with the design system's local `var(--wppo-bg-card)` variable, which currently points to white but provides a standardized foundation for future dynamic adaptations. Ensure that `!important` modifiers are preserved when refactoring form inputs to prevent WordPress core CSS from overriding them.
