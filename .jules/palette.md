...
## 2024-05-18 - Replacing hardcoded white backgrounds with design system variables
**Learning:** React components inside `.wppo-dialog`, `.wppo-audit-controls` and form elements hardcoded backgrounds to `#fff` or `white` instead of the local variable `--wppo-bg-card` available to WP, which could cause display issues when integrating closely with darker or customized WordPress admin color schemes.
**Action:** Use `var(--wppo-bg-card)` for all internal card backgrounds, dialogs, and elements originally given a white hex or string. This standardizes UI rendering and maintains consistency with WP theme integrations.
