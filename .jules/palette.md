...
**Action:** When auditing or building new forms, I must always prefer the reusable `CheckboxOption` component (or the `useId()`-based pattern) over hand-adding `id` + `aria-describedby`, keeping the manual fallback only for custom inputs the shared components do not cover. This avoids manual wiring and ID-collision risk.
