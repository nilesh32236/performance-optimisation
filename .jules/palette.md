...
## 2024-06-28 - Refactored hardcoded white backgrounds to WP CSS variables
**Learning:** Hardcoded white backgrounds (`#fff` or `white`) in SCSS components (e.g., `background: #fff;`) break dynamic theme adaptations since native WP admin themes typically keep card backgrounds light but might require subtle shifts.
**Action:** Replace hardcoded white backgrounds with `var(--wppo-bg-card)` or similar local design system variables (which map to WP variables) to standardize CSS and enable deeper dynamic adaptations.
