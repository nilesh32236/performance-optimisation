...
## 2026-08-29 - Edge Cache ARIA Labels
**Learning:** Missing `aria-describedby` links between inputs (and select dropdowns) and their helper texts can break screen reader context in React components.
**Action:** Always ensure any `<p className="description">` or helper text is linked to its input using `aria-describedby` and an id.
