...
**Test Added:** Replaced `array_filter(..., 'strlen')` with strictly typed closures `static function (mixed $val): bool`. Corrected regex capture group extraction logic to explicitly prefer group 2 first and fallback to group 3: `$type = '' !== $type_matches[2] ? $type_matches[2] : $type_matches[3];` (prefers quoted group 2, no redundant `??` as PCRE guarantees offsets). Note: No unit tests were added in this PR as the scope was strictly constrained to static analysis type-safety fixes (coverage for typed closures and quoted/unquoted extractions is deferred to PHPStan).
