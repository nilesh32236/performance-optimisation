...
## 2026-05-24 - [JS Test Fix] Testing API error logging
**Bug/Gap:** Tests triggering expected errors on 'sad paths' pollute the terminal with console.error logs, obscuring real failures.
**Root Cause:** Errors generated by mocked APIs were explicitly logged in components without being caught in the test environment.
**Test Added:** Mocked `console.error` using `jest.spyOn(console, 'error')` inside the sad path test to silence expected outputs, and restored it with `mockRestore()` after assertion to keep test execution clean.
