...
**Action:** Extract a single shared helper `Util::cached_content_url( $path )` that keys a static array by `get_current_blog_id()`, only caches when no `content_url` filter is registered (matching the `class-main.php` convention), and use it from every minifier call site.
