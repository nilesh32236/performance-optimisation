...
if grep -q 'CONTEXT7_API_KEY:' "${tri_workflow}" || grep -q 'git remote add origin-temp' "${tri_workflow}" || ! grep -q 'env -i' "${tri_workflow}" || ! grep -q 'rm -f .git/FETCH_HEAD' "${tri_workflow}" || ! grep -q 'OPENCODE_API_KEY_VALUE=' "${tri_workflow}" || ! grep -q 'unset GH_TOKEN OPENCODE_API_KEY OPENAI_API_KEY' "${tri_workflow}"; then
