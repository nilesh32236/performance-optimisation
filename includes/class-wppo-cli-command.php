...
if ( ! class_exists( 'PerformanceOptimise\Inc\WPPO_CLI_Command' ) ) {
	...
	class WPPO_CLI_Command extends WP_CLI_Command {
		...
		public function database( array $args, array $assoc_args ): void {
			...
			if ( 'optimize' === $action ) {
				...
				foreach ( $table_list as $table ) {
					if ( '' === $table || ! in_array( $table, $allowed_tables, true ) ) {
						...
						WP_CLI::warning( sprintf( __( ' - Skipped unknown table: %s', 'performance-optimisation' ), sanitize_text_field( $table ) ) );
						continue;
					}
					...
					if ( $result ) {
						++$success_count;
						/* translators: %s: Table name */
						WP_CLI::log( sprintf( __( ' - Optimized table: %s', 'performance-optimisation' ), sanitize_text_field( $table ) ) );
					}
				}
			...
			if ( 'all' === $type ) {
				...
				foreach ( $results as $key => $val ) {
					...
					$count  = (int) $val;
					$total += $count;
					/* translators: 1: Cleanup type, 2: Item count */
		...
		public function pagespeed( array $args, array $assoc_args ): void {
			$action   = $args[0] ?? 'scan';
			$url      = isset( $assoc_args['url'] ) && is_string( $assoc_args['url'] ) ? esc_url_raw( $assoc_args['url'] ) : Util::cached_home_url();
			$strategy = isset( $assoc_args['strategy'] ) && is_string( $assoc_args['strategy'] ) ? $assoc_args['strategy'] : 'mobile';
