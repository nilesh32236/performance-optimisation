...
class ImageService implements ImageServiceInterface {
	...
	public function convert_image( string $source_image_path, string $target_format = 'webp' ): string {
		...
		try {
			...
			if ( $success ) {
				...
				LoggingUtil::info( 'Image conversion successful', array(
				...
				LoggingUtil::error( 'Image conversion failed', array(
					'source' => $source_image_path,
			...
			$this->conversionQueue->update_status( $source_image_path, $target_format, 'failed' );
			LoggingUtil::error( 'Image conversion exception: ' . $e->getMessage(), array(
				'source' => $source_image_path,
				'target_format' => $target_format,
			) );
			return '';
	...
	public function process_uploaded_image( int $attachment_id ): void {
		...
		if ( ! ImageUtil::isImageFormat( $file_path ) ) {
			LoggingUtil::info( 'Skipping non-image attachment', array( 'attachment_id' => $attachment_id, 'path' => $file_path ) );
			return;
		}
		...
		if ( $added_to_queue > 0 ) {
			...
			LoggingUtil::info( 'Image added to conversion queue', array(
	...
	public function get_conversion_stats(): array {
		...
		return array_merge( $queue_stats, array(
	...
	public function cleanupOrphanedImages(): array {
		...
		if ( ! FileSystemUtil::fileExists( $wppo_dir ) ) {
			return array( 'cleaned' => 0, 'space_freed' => 0 );
		}
		...
		LoggingUtil::info( 'Orphaned image cleanup completed', array(
