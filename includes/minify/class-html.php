	...
	class HTML {
		...
		public function __construct( $html, $options ) {
			...
			foreach ( $priority_raw as $line ) {
				...
				if ( 2 === count( $parts ) ) { // Audit #1362: Yoda (WPCS).
					$handle = trim( $parts[0] );
		...
		private static function compile_delay_exclude_re( array $exclusions ): ?string {
			try {
				...
				set_error_handler( static function () {} ); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_set_error_handler -- Suppress warnings from validating the generated alternation.
				try {
					$valid = preg_match( $re, '' );
				} catch ( \Throwable $e ) {
					unset( $e );
