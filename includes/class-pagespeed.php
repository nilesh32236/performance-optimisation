	...
	class Pagespeed {
		...
		public static function run_scan( array $args ): void {
			...
			if ( false !== strpos( $request_url, 'localhost' ) || false !== strpos( $request_url, '127.0.0.1' ) ) {
				new Log( 'PageSpeed scan: localhost URL detected, substituting https://activewebsitemanagement.com/ for API call.' );
				$request_url = 'https://activewebsitemanagement.com/';
			}

			$query_args = array(
				'url'      => rawurlencode( $request_url ),
				'key'      => $api_key,
