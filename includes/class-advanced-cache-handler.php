...
if ( ! class_exists( 'PerformanceOptimise\Inc\Advanced_Cache_Handler' ) ) {
	...
	class Advanced_Cache_Handler {
		...
		public static function create(): bool {
			...
			'// This helper fires a rate-limited, non-blocking loopback to wp-cron.php.' . PHP_EOL .
			'// Pre-boot constraints: pure PHP only — no Util, sanitize_*, wp_*' . PHP_EOL .
			'// or filter calls here. Fail-open everywhere: the helper must never' . PHP_EOL .
			'// fatal or delay a cache hit. Kill-switch: define(' . PHP_EOL .
			'// WPPO_DROPIN_DISABLE_CRON_SPAWN=true to disable; override the 60s' . PHP_EOL .
			'// rate-limit with WPPO_DROPIN_CRON_INTERVAL (seconds, min 10).' . PHP_EOL .
			'function wppo_maybe_spawn_cron( $site_url, $canonical_host ) {' . PHP_EOL .
			'	if ( defined( \'WPPO_DROPIN_DISABLE_CRON_SPAWN\' ) && WPPO_DROPIN_DISABLE_CRON_SPAWN ) {' . PHP_EOL .
			'		return;' . PHP_EOL .
			'	}' . PHP_EOL .
			'	if ( defined( \'DISABLE_WP_CRON\' ) && DISABLE_WP_CRON ) {' . PHP_EOL .
			'		return;' . PHP_EOL .
			'	}' . PHP_EOL .
			...
			'		return;' . PHP_EOL .
			'	}' . PHP_EOL .
			'	if ( is_string( $canonical_host ) && \'\' !== $canonical_host && strtolower( (string) $parts[\'host\'] ) !== strtolower( $canonical_host ) ) {' . PHP_EOL .
			'		return;' . PHP_EOL .
			'	}' . PHP_EOL .
			'	$scheme = isset( $parts[\'scheme\'] ) ? strtolower( (string) $parts[\'scheme\'] ) : \'http\';' . PHP_EOL .
			'	$is_ssl = ( \'https\' === $scheme || \'ssl\' === $scheme );' . PHP_EOL .
			'	$host = (string) $parts[\'host\'];' . PHP_EOL .
			'	$port = isset( $parts[\'port\'] ) ? (int) $parts[\'port\'] : ( $is_ssl ? 443 : 80 );' . PHP_EOL .
			'	if ( $port <= 0 || $port > 65535 ) {' . PHP_EOL .
			'		return;' . PHP_EOL .
			'	}' . PHP_EOL .
			...
			'function wppo_serve_cache_file( $file_path, $gzip_file_path, $brotli_file_path, $cache_life, $accept_encoding, $site_url, $canonical_host ) {' . PHP_EOL .
			...
			'	}' . PHP_EOL .
			'	if ( false !== strpos( $accept_encoding, \'br\' ) && file_exists( $brotli_file_path ) ) {' . PHP_EOL .
			'		wppo_maybe_spawn_cron( $site_url, $canonical_host );' . PHP_EOL .
			...
			'	wppo_serve_cache_file( $file_path, $gzip_file_path, $brotli_file_path, $cache_life, $accept_encoding, $site_url, $canonical_host );' . PHP_EOL .
			'}' . PHP_EOL . PHP_EOL .
			'if ( $is_logged_in && ! $has_query ) {' . PHP_EOL .
			...
			'		wppo_serve_cache_file( $role_file_path, $role_gzip_path, $role_brotli_path, $cache_life, $accept_encoding, $site_url, $canonical_host );' . PHP_EOL .
			'	}' . PHP_EOL .
			'}' . PHP_EOL;
