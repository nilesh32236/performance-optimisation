...
if ( ! class_exists( 'PerformanceOptimise\Inc\Cache' ) ) {
	...
	class Cache {
		...
		/**
		 * Lazily-created capacity/accounting runner (P3-005).
		 *
		 * Owns statistics, cap settings, byte/file checks, randomized-query
		 * detection, and oldest-entry eviction while this class keeps the
		 * public compatibility facade and lifecycle/storage policy.
		 *
		 * @since NEXT
		 * @var Cache_Capacity|null
		 */
		private ?Cache_Capacity $cache_capacity = null;
		 ...
		 */
		private function capacity(): Cache_Capacity {
			if ( null === $this->cache_capacity ) {
				$this->cache_capacity = new Cache_Capacity( $this );
			}
		...
		/**
		 * Filesystem for the capacity/accounting service (P3-005 internal bridge).
		 *
		 * @internal
		 * @access private
		 * @since NEXT
		 * @return object|false|null Filesystem object, or false on init failure.
		 */
		...
		/**
		 * Cache root for the capacity/accounting service (P3-005 internal bridge).
		 *
		 * @internal
		 * @access private
		 * @since NEXT
		 * @return string Canonical cache root directory.
		 */
		...
		/**
		 * Cache domain for the capacity/accounting service (P3-005 internal bridge).
		 *
		 * @internal
		 * @access private
		 * @since NEXT
		 * @return string Canonical cache domain.
		 */
		...
		/**
		 * Path-containment verdict for capacity eviction (P3-005 internal bridge).
		 *
		 * @internal
		 * @access private
		 * @since NEXT
		 * @param string $path Absolute file or directory path.
		 * @return bool True when the path is contained.
		 */
		...
		/**
		 * Delete a cache entry through the invalidation owner (P3-005 internal bridge).
		 *
		 * Keeps storage/invalidation policy on Cache_Invalidator while capacity
		 * eviction retains the existing sibling-aware deletion behavior.
		 *
		 * @internal
		 * @access private
		 * @since NEXT
		 * @param string $file_path Cache entry path.
		 ...
		 * @return string
		 * @since 1.0.0
		 * @since NEXT Proxied to Cache_Capacity (P3-005).
		 */
		public static function get_cache_size(): string {
			return ( new self() )->capacity()->get_cache_size();
		}
		...
		public static function get_cache_stats(): array {
			return ( new self() )->capacity()->get_cache_stats();
		}

		/**
		 * Store the unified cache-stats payload.
		 *
		...
		private static function store_cache_stats( array $unified, string $stats_key ): void {
			( new self() )->capacity()->store_cache_stats( $unified, $stats_key );
		}

		/**
		 * List direct cache children for capacity accounting.
		 *
		...
		private function list_cache_children( string $directory ): ?array {
			return $this->capacity()->list_cache_children( $directory );
		}

		/**
		 * Calculate directory size and cached-page count in one walk.
		 *
		...
		private function calculate_directory_stats( string $directory, int $depth = 0 ): array {
			return $this->capacity()->calculate_directory_stats( $directory, $depth );
		}
		...
		private function calculate_directory_size( string $directory, int $depth = 0 ): int {
			return $this->capacity()->calculate_directory_size( $directory, $depth );
		}

		/**
		 * Count cached pages by counting index.html files.
		 *
		 * @param string $directory Directory to scan.
		 * @param int    $depth     Recursion depth.
		 * @return int Cached page count.
		 * @since 1.9.0
		 * @since NEXT Proxied to Cache_Capacity (P3-005).
		 */
		private function count_cached_pages( string $directory, int $depth = 0 ): int {
			return $this->capacity()->count_cached_pages( $directory, $depth );
		}

		/**
		 * Read bounded-cache cap settings with fail-safe defaults.
		 *
		...
		public static function get_cache_cap_settings(): array {
			return Cache_Capacity::get_cache_cap_settings();
		}

		/**
		 * Get total static-cache bytes and file count in one walk.
		 *
		 * @return array{bytes:int,files:int} Bytes and file count.
		 * @since 2.3.0
		 * @since NEXT Proxied to Cache_Capacity (P3-005).
		 */
		public static function get_cache_bytes_and_files(): array {
			return ( new self() )->capacity()->get_cache_bytes_and_files();
		}

		/**
		 * Get total static-cache size in bytes.
		 *
		 * @return int Bytes used, or zero on failure.
		 * @since 2.2.0
		 * @since NEXT Proxied to Cache_Capacity (P3-005).
		 */
		public static function get_cache_size_bytes(): int {
			return ( new self() )->capacity()->get_cache_size_bytes();
		}

		/**
		 * Get total cached-page file count.
		 *
		 * @return int File count, or zero on failure.
		 * @since 2.3.0
		 * @since NEXT Proxied to Cache_Capacity (P3-005).
		 */
		public static function get_cache_file_count(): int {
			return ( new self() )->capacity()->get_cache_file_count();
		}

		/**
		 * Whether an asset URL carries randomized per-request query churn.
		 *
		 * @param string $src Asset src URL.
		 * @return bool Whether the query looks randomized.
		 * @since 2.3.0
		 * @since NEXT Proxied to Cache_Capacity (P3-005).
		 */
		public static function is_randomized_query_asset( string $src ): bool {
			return Cache_Capacity::is_randomized_query_asset( $src );
		}

		/**
		 * Get warn-before-enforce cache capacity status.
		 *
		...
		public static function get_cache_cap_status(): array {
			return ( new self() )->capacity()->get_cache_cap_status();
		}

		/**
		 * Throttled warn-before-enforce cap check after a cache write.
		 *
		 * @return void
		 * @since 2.2.0
		 * @since NEXT Proxied to Cache_Capacity (P3-005).
		...
		public static function maybe_enforce_cache_cap(): void {
			( new self() )->capacity()->maybe_enforce_cache_cap();
		}

		/**
		 * Evict oldest cached pages until the byte target is freed.
		 *
		...
		public static function evict_oldest_cache_entries( int $bytes_to_free ): int {
			return ( new self() )->capacity()->evict_oldest_cache_entries( $bytes_to_free );
		}

		/**
		 * Evict oldest cached pages until the file-count target is met.
		 *
		 * @param int $files_to_free Minimum entries to remove.
		 * @return int Entries actually removed.
		 * @since 2.3.0
		 * @since NEXT Proxied to Cache_Capacity (P3-005).
		 */
		public static function evict_oldest_cache_files_by_count( int $files_to_free ): int {
			return ( new self() )->capacity()->evict_oldest_cache_files_by_count( $files_to_free );
		}

		/**
		 * Collect index.html entries with mtime and size for oldest eviction.
		 *
		 * @param string $directory Directory to scan.
		 * @param int    $depth     Recursion depth.
		...
		private function collect_cache_entries_by_age( string $directory, int $depth = 0, array &$out = array() ): array {
			return $this->capacity()->collect_cache_entries_by_age( $directory, $depth, $out );
		}
