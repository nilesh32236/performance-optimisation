...
class Object_Cache {
	...
	public function enable( $config ) {
		...
		// Replace password with a constant reference so the secret isn't in plaintext.
		$has_password = isset( $config['password'] ) && '' !== $config['password'];
		if ( $has_password ) {
		...
		if ( $has_password ) {
			// Unquote the constant reference so the drop-in resolves the constant at runtime.
