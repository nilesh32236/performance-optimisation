...
namespace {
	...
	class MockWPDB {
		...
		public function get_results( $query, $output = OBJECT ) {
			if ( strpos( $query, 'SHOW TABLES' ) !== false ) {
				return array( array( 'wp_posts' ), array( 'wp_comments' ), array( 'wp_options' ) );
			}
			return array();
		}
	}
	...
	$settings->set_mock_setting( 'database', 'cleanup', array( 'revisions' => true ) );
	...
	// Test 2: Cleanup Spam
	echo "\nTest 2: Cleanup Spam\n";
	$settings->set_mock_setting( 'database', 'cleanup', array( 'spam' => true ) );
	$result = $service->run_cleanup();
	...
	echo "\nTest 3: Cleanup Trash\n";
	$settings->set_mock_setting( 'database', 'cleanup', array( 'trash' => true ) );
	$result = $service->run_cleanup();
	...
	echo "\nTest 4: Optimize Tables\n";
	$settings->set_mock_setting( 'database', 'cleanup', array( 'optimize_tables' => true ) );
	$result = $service->run_cleanup();
