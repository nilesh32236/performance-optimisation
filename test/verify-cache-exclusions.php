...
namespace {
	...
	test_exclusion( '/some-page', array(), true, 'No exclusions should cache' );

	// Test 2: Exact URL Match
	test_exclusion( '/no-cache', array( 'urls' => array( 'no-cache' ) ), false, 'Exact URL match should skip' );
	test_exclusion( '/cache-me', array( 'urls' => array( 'no-cache' ) ), true, 'Non-matching URL should cache' );

	// Test 3: Wildcard URL Match
	test_exclusion( '/shop/product-1', array( 'urls' => array( 'shop/*' ) ), false, "Wildcard match 'shop/*' should skip" );
	test_exclusion( '/blog/post-1', array( 'urls' => array( 'shop/*' ) ), true, 'Non-matching wildcard should cache' );

	// Test 4: Cookie Exclusion
	$_COOKIE['woocommerce_items_in_cart'] = '1';
	test_exclusion( '/cart', array( 'cookies' => array( 'woocommerce_items_in_cart' ) ), false, 'Cookie presence should skip' );
	unset( $_COOKIE['woocommerce_items_in_cart'] );
	test_exclusion( '/cart', array( 'cookies' => array( 'woocommerce_items_in_cart' ) ), true, 'Cookie absence should cache' );

	// Test 5: User Agent Exclusion
	$_SERVER['HTTP_USER_AGENT'] = 'Googlebot/2.1';
	test_exclusion( '/', array( 'user_agents' => array( 'Googlebot' ) ), false, 'User Agent match should skip' );
	$_SERVER['HTTP_USER_AGENT'] = 'Mozilla/5.0';
	test_exclusion( '/', array( 'user_agents' => array( 'Googlebot' ) ), true, 'User Agent non-match should cache' );
