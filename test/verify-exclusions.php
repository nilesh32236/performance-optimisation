...
namespace {
	...
	// Test 1: Exact Match
	test_match( 'jquery', array( 'jquery' ), true, "Exact match 'jquery'" );
	test_match( 'jquery', array( 'other' ), false, "No match 'jquery' vs 'other'" );

	// Test 2: Wildcard Match
	test_match( 'wp-block-library', array( 'wp-*' ), true, "Wildcard match 'wp-*' matches 'wp-block-library'" );
	test_match( 'custom-style', array( '*-style' ), true, "Wildcard match '*-style' matches 'custom-style'" );
	test_match( 'script.js', array( '*.js' ), true, "Wildcard match '*.js' matches 'script.js'" );
	test_match( 'style.css', array( '*.js' ), false, "Wildcard match '*.js' does not match 'style.css'" );

	// Test 3: Regex Match
	test_match( 'custom-123-script', array( '/custom-\d+-script/' ), true, "Regex match '/custom-\d+-script/'" );
	test_match( 'custom-abc-script', array( '/custom-\d+-script/' ), false, 'Regex match fail' );

	// Test 4: Substring Match (Legacy)
	test_match( 'https://example.com/wp-content/plugins/my-plugin/style.css', array( 'my-plugin' ), true, "Substring match 'my-plugin'" );

	// Test 5: Multiple Patterns
	$patterns = array( 'jquery', 'wp-*', '/custom-\d+/' );
