...
const DashboardChart: React.FC<DashboardChartProps> = ({
	...
	const getTrendIndicator = (data: ChartData) => {
		if (data.daily_trends.length < 2) return null;

		const firstValue = data.daily_trends[0].value;
		const lastValue = data.daily_trends[data.daily_trends.length - 1].value;
		...
		const improvement = isImprovement(data.metric_name, change);
		const absChange = Math.abs(change);

		if (absChange < 1) return null; // No significant change

		return (
			<div className={`wppo-trend-indicator wppo-trend-indicator--${improvement ? 'positive' : 'negative'}`}>
				<span className={`dashicons ${improvement ? 'dashicons-arrow-up-alt' : 'dashicons-arrow-down-alt'}`}></span>
				<span className="wppo-trend-value">{absChange.toFixed(1)}%</span>
				<span className="wppo-trend-label">
					{improvement ? 'improved' : 'declined'}
				</span>
			</div>
		);
	};
