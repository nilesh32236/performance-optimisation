...
( function() {
	...
	function isInViewport( el ) {
		...
		const inView =
			rect.top <=
				( window.innerHeight || document.documentElement.clientHeight ) + LAZY_OFFSET &&
			rect.bottom >= -LAZY_OFFSET &&
			rect.left <=
				( window.innerWidth || document.documentElement.clientWidth ) + LAZY_OFFSET &&
	...
	function lazyLoadIframe( iframe ) {
		if ( iframe.dataset.wppoLoaded === 'true' ) {
			return;
		}
	...
	function lazyLoadVideo( video ) {
		if ( video.dataset.wppoLoaded === 'true' ) {
			return;
		}
	...
	function processLazyElements() {
		...
		document
			.querySelectorAll( 'iframe.wppo-lazy-iframe[data-src]:not([data-wppo-loaded="true"])' )
			.forEach( ( iframe ) => {
		...
		document
			.querySelectorAll( 'video.wppo-lazy-video:not([data-wppo-loaded="true"])' )
			.forEach( ( video ) => {
	...
	function cleanupEventListeners() {
		...
		if (
			remainingLazyImages === 0 &&
			remainingLazyIframes === 0 &&
			remainingLazyVideos === 0 &&
			remainingDelayedScripts === 0
		) {
			...
			window.removeEventListener( 'orientationchange', throttledProcessLazyElements, {
				passive: true,
			} );
		} else {
		}
	...
	if ( isBot || ( document.body && document.body.classList.contains( 'logged-in' ) ) ) {
		document
			.querySelectorAll( 'script[type="wppo/javascript"]:not([data-wppo-processed="true"])' )
			.forEach( ( script ) => {
				executeDelayedScript( script );
			} );
		return;
