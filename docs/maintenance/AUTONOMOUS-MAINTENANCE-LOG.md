...
- **A latent fixture defect this surfaced.** `SchedulerCronPrimitivesTest::all_features_enabled_settings()` is named "all features enabled" but omitted `criticalCSS`. Because the schedule was unconditional, the omission was invisible: the test passed while the feature was in fact off. Gating the schedule exposed it. The fixture now actually enables the feature, so the existing expectation stands unchanged and the two tests together cover both directions.
- Gate: PHPCS 0, PHPUnit 2,812 / 25,974, ESLint 0, Jest 60 suites / 1,072, architecture `--check` 0, `git diff --check` 0. Mutation re-checked after the fixture fix: reverting to the unconditional schedule still fails 3 cases.

## 2026-09-26 — The crawler ability's plural `urls` input fatals on PHP 8
- Extracted from #1664 after an independent adversarial review found that PR carried four behaviour-changing hunks and could not be merged as-is. This is the one hunk the review identified as "a genuine fix" with real value, so it was reimplemented on a fresh base with tests the original did not have.
- `Abilities::execute_crawler()` maps `esc_url_raw` over `$input['urls']`. On PHP 8 that is a `TypeError` for any non-string element, including a nested array. The **single** `url` case directly above was already guarded, carrying a comment naming this exact reason: *"Audit #1434: array input fatals esc_url_raw on PHP 8 — skip non-strings like resolve_input_url() does."* The plural case was simply missed, so one bad element took down the whole crawler run instead of skipping that element.
- Fix: `array_filter( $input['urls'], 'is_string' )` before the map, matching the sibling guard.
- `AbilitiesCrawlerInputTest` observes the gathered URL set through the `wp_http_validate_url` filter, which is applied to every URL after sanitising. The stub records and returns false, so the method returns "No valid URLs to crawl." *before* dispatching to the real `LiteSpeed_Crawler::crawl_batch()` — the test asserts that early return, so it can never quietly start doing real network work. The `esc_url_raw` stub throws on a non-string rather than coercing, so the test double cannot mask the very regression it exists to catch.
