...
## 2026-09-25 — #1563 Installer Recovery
- Active branch: `fix/wordpress-monitor-installer-1563`, pushed to origin with the installer security fix and regression hardening; no PR was created because approval-gated GitHub writes are unavailable.
- Root causes: run `35958652063` installed successfully but failed research with the then-active `opencode/space-bunny-free` route; run `36140868928` failed before research because the pinned installer called the GitHub Releases API and masked the transport error.
- Repair: direct pinned v1.18.29 URLs, committed x64/arm64 checksums, archive/type/size validation, staged exact-version validation, atomic rollback-safe installation, credential scrubbing, `GITHUB_PATH` publication, a dedicated stubbed-network regression job, and tri-merge conflict resolution that uses ephemeral Git auth, clears fetch metadata, and runs OpenCode with a minimal environment.
- Verification: shell syntax, installer regression, real isolated v1.18.29 x64 smoke, 59 Jest suites/833 tests, 2,783 PHPUnit tests/25,808 assertions, PHPCS, build, architecture generator, and diff checks all passed. Hosted CI and a merged-head monitor rerun remain pending human-gated work.
- Scope boundary: no model-routing change; Muse Spark remains the explicit champion. Floating third-party action installers and model-promotion safety are separate follow-up work.
- Verified follow-up queue: tri-merge must re-query checks after force-pushing resolved heads and use deterministic generated-build conflict policy; release discovery must fail closed on GitHub API errors and never delete an existing tag; monitor publisher failures need notification; model promotion needs fail-closed free-registry admission.

## 2026-09-25 — CI command-injection and fail-closed merge/release repair
- Active branch (unchanged): `fix/wordpress-monitor-installer-1563`. The unauthenticated fork-PR RCE outranked the standing wait-for-gate preference, so the bounded fix was folded into this branch instead of opening a competing one; the human PR/merge gate is still required and nothing was merged, approved, or labelled.
- Critical (verified empirically by a fresh read-only subagent): `echo "ref=${{ github.event.pull_request.head.ref }}"` in three `wppo-ai-review.yml` `Resolve PR ref` steps executed an attacker-chosen `$( )`/backtick substitution on the runner before checkout, with `GITHUB_TOKEN` in scope. `"` is illegal in git refs; `$( )` and backticks are not.
- Fixed: `PR_HEAD_REF`, `PR_TITLE`, `BRANCH` and `FORCE_VERSION` now travel through `env:`; non-empty resolved refs are validated with `git check-ref-format`; an empty ref still falls through to checkout's default so the manual-dispatch path is unchanged.
- Fixed (fail-closed merge/release): auto-merge now computes `total`, refuses zero-check PRs, stops treating `SKIPPED`/`NEUTRAL` as green, aborts when the head SHA moves during polling, and merges with `--match-head-commit`; tri-merge re-validates draft state and check rollup at merge time, propagates merge failures, gates `release` on `merge-all`, anchors the version-bump sed to the ` * Version:` header, asserts header/define/package agreement, and refuses to delete or re-create an existing release tag.
- Fixed (docs): `AGENTS.md` no longer lists the three non-existent `qoder-*`/`tri-merge-cycle` workflows or the never-implemented `daily-audit` 95% auto-merge.
- New regression: `.github/scripts/tests/workflow-untrusted-context.sh` (hermetic; static scan, positive assertions, `x$(id)` canary, mutation control, nine-case jq matrix, version-bump fixture), wired into the existing `webpack.yml` shell-regression job which is now named "CI shell regressions (installer + workflows)". No branch protection exists on this repository, so no required-check name was orphaned.
- Mutation-checked: restoring the inline interpolation fails the scanner; restoring `SKIPPED`/`NEUTRAL` to the good list fails the jq matrix; the old `^Version:` sed leaves the header untouched.
- Scope boundary: no model/provider/routing change (Muse Spark remains explicit), no public API change, no changes to plugin PHP/JS sources.
