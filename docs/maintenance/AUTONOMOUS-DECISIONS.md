...
## CI Command-Injection and Fail-Open Merge/Release Decisions 2026-09-25
- **Severity drives the branch, not a new issue:** A fresh read-only auditor empirically confirmed that `github.event.pull_request.head.ref` was interpolated directly into a `run:` body in three `wppo-ai-review.yml` jobs. `"` is illegal in git refs but `$( )` and backticks are legal, and `pull_request` fires for fork PRs, so an unauthenticated third party could execute a command substitution on a runner before checkout, with `GITHUB_TOKEN` in the step environment. A verified unauthenticated-RCE path outranks the standing "wait for the human gate" preference, and opening a second implementation branch would have violated the one-active-implementation rule, so the bounded fix was folded into the existing #1563 branch and reported explicitly rather than silently.
- **Fix shape:** pass every untrusted value through `env:` and read the variable inside the script. Numeric ids (`pull_request.number`) and git SHAs (`head.sha`) are validated by GitHub and cannot carry shell metacharacters, so they stay inline; the scanner encodes exactly that distinction so the rule stays precise enough to keep.
- **Bounded extra hardening in the same surface:** the same pass fixed the PR title and `github.ref_name` interpolations, added `git check-ref-format` on non-empty resolved refs, validated the `force_version` override as `MAJOR.MINOR.PATCH`, and made the auto-merge and tri-merge gates fail closed (zero-check rollups, `SKIPPED`/`NEUTRAL`, head-SHA movement, drafts, merge failure propagation, and the release dependency on `merge-all`). Each change is behaviour-preserving for the normal path and fail-closed otherwise; no route, model, or public API changed.
- **Release safety:** the version bump sed matched `^Version:`, which the ` * Version:` header never satisfies, so the published version froze while only `WPPO_VERSION` advanced. The next cycle therefore re-tagged the already published `v2.4.0` and deleted it from the remote. The bump is now anchored to the comment form, preserves column alignment, and asserts that header, `WPPO_VERSION` and `package.json` agree before tagging; an existing tag is a hard failure instead of a delete-and-recreate.
- **Documentation:** `AGENTS.md` listed three workflows that do not exist (`qoder-auto-review.yml`, `qoder-assistant.yml`, `tri-merge-cycle.yml`) and claimed `daily-audit.yml` auto-merges at 95% confidence, which it never did. The table and the merge-gate rule now describe the real, fail-closed control and name the absent human gate.
- **Regression evidence:** `.github/scripts/tests/workflow-untrusted-context.sh` is hermetic. It scans every `run:` body, asserts the env pattern survives, executes the real `Resolve PR ref` step with a `x$(id)` canary to prove literal copying, reintroduces the vulnerable line to prove the detector fires, runs the extracted auto-merge jq against a nine-case check matrix, and applies the extracted version-bump expression to a header fixture. Both the injection fix and the merge-gate fix were confirmed to fail the test when reverted.
