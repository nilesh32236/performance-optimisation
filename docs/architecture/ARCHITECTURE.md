...
| `Runtime_State` | Central switch_blog reset registry for six site-sensitive owners plus the OD_Bridge request-local memo | Keep reset methods feature-owned; classify the remaining static owners |
...
2. **Runtime state:** 32 owners hold static state; the six site-sensitive owners plus the OD_Bridge request-local memo now reset centrally, while the remainder need classification.
