...
P3-008 adds the dependency-light `Settings_Command` as the single write-orchestration seam. `Rest_Settings` partial saves, imports, and restores plus the REST safe-mode toggle route through it; the command snapshots-if-changed (fail-open, enable-only rule for safe mode) and writes only via `Settings_Store::save_settings()`, which remains the sole direct `wppo_settings` writer. The safe-mode enable payload builder is caller-injected so the command keeps no upper-layer edge to `Main`. The generated inventory now contains 78 files, 81 graph nodes, and 355 edges; boundary violations stay at 16, bridge candidates at 230, and feature-to-feature edges at 46.
...
Util migration, settings writes, REST/CLI application boundaries, CSS and image decompositions, and React card extraction remain queued. Cache capacity, runtime-state reset ownership, Redis configuration policy, and neutral drop-in invalidation are implemented in this phase.
