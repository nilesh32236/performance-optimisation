...
P3-013 trades three explicit `Main` owner edges for a 301-line and one-large-method reduction. P3-014 owns the remaining asset/minification/speculation fan-out reduction; P3-016 now replaces the `Settings_Migrations` → `Main` owner-state bridge with callable effective-options and invalidation contracts. `Settings_Store` adds two blog-keyed resolved-memo properties, while incoming executable `Util` evidence falls 1,142 to 1,138. Moving the canonical defaults into the dependency-light Store prevents a new Store-to-Util edge; the runtime SCC grows only from 64 to 65 nodes. P3-017 migrates the warm-path Woo caller cluster (`Cron`, `Preload_Buffer_Coordinator`) to `Woo_Detect`: incoming executable `Util` evidence falls 1,138 to 1,110 while `Woo_Detect` fan-in grows 1 to 3, adding two documented owner edges (and two bridge candidates) with all `Util` proxies retained as compatibility shims.
