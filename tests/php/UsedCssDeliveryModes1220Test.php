...
class UsedCssDeliveryModes1220Test extends \PHPUnit\Framework\TestCase {
	 ...
	 *
	 * Stubs the Action Scheduler enqueue function so the test reaches the
	 * cooldown gate instead of passing vacuously on the missing-dependency
	 * early return (issue #1220). The targeted-regen timestamp is fresh
	 * ($now), so the cooldown gate — not the missing dependency — yields 0.
	 */
	public function test_targeted_regen_respects_cooldown(): void {
		...
		Functions\when( 'has_filter' )->justReturn( false );
		// Reach the cooldown gate: without these stubs request_targeted_regen()
		// early-returns 0 on the missing Action Scheduler dependency.
	...
	/**
	 * Strict-CSP detection covers <meta http-equiv> tags, not just headers.
	 *
	 * A meta-delivered CSP without 'unsafe-inline' blocks the async onload
	 * swap and the delay inline loader alike, so it must trigger the
	 * file-mode downgrade (issue #1220).
	 */
	public function test_has_strict_csp_detects_meta_tag(): void {
		...
		$permissive_meta = '<html><head><meta http-equiv="Content-Security-Policy" content="script-src \'self\' \'unsafe-inline\'"></head><body></body></html>';
		$this->assertFalse( $method->invoke( $instance, $permissive_meta ) );

		$this->assertFalse( $method->invoke( $instance, '<html><head></head><body></body></html>' ) );
