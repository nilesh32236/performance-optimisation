...
class SpeculationPrerenderListTest extends \PHPUnit\Framework\TestCase {
	...
	/**
	 * Site-wide commerce signal (WooCommerce installed) no longer suppresses
	 * the prerender rule (issue #1243): safe pages still prerender while
	 * cart/checkout/account URLs stay excluded per URL.
	 *
	 * @return void
	 */
	public function test_commerce_presence_still_emits_safe_urls(): void {
		$this->install_settings( $this->prerender_on() );
		$main = $this->make_main( $this->prerender_on() );
	...
	/**
	 * Request-level commerce pages still get no prerender rule (issue #1243):
	 * a visitor on the cart page is suppressed via
	 * is_speculation_suppressed_for_visitor().
	 *
	 * @return void
	 */
	public function test_cart_page_visitor_gets_no_prerender_rule(): void {
		$this->install_settings( $this->prerender_on() );
		$main = $this->make_main( $this->prerender_on() );
		...
		try {
			...
			// Brain Monkey eval-declares is_cart() process-wide: restore the
			// default so later tests in this process see a non-cart visitor.
