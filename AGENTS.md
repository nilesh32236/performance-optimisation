...
├── DatabaseCleanup.js (9 types via CLEANUP_METHOD_MAP in includes/Database/class-database-cleanup.php)
...
| `class-util.php` (root) | Filesystem, URL processing, preload links, MIME types |
| `Support/class-log.php` | Activity logging to `wppo_activity_logs` table |
...
- **AI fixer authorization (issue #1564)**: `/fix` comment triggers in `wppo-ai-review.yml` (`fix-issue`, `autofix`) accept only `OWNER`/`MEMBER`/`COLLABORATOR` via `github.event.comment.author_association`; label triggers (`autofix-trigger`, `autofix`) are maintainer-controlled via label permission. Never widen a write-capable (`mode: fix`) path to untrusted commenters.
